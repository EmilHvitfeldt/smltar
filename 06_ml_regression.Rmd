...
```{r rfpredictoffline, dependson="scotusrfrsoffline", opts.label = "fig.square", eval=!online}
rf_rs %>%
  collect_predictions() %>%
  ggplot(aes(year, .pred, color = id)) +
  geom_abline(lty = 2, color = "gray80", size = 1.5) +
...
```{r rfpredict, echo = FALSE, opts.label = "fig.large", fig.cap="The random forest model did not perform very sensibly across years, compared to our first attempt using a linear SVM model"}
...
```{r scotusfinalpredvis, dependson="finalscotusrs", opts.label = "fig.large", fig.cap="Predicted and true years from a linear SVM regression model with bigrams and unigrams, and stop words removed"}
final_fitted %>%
  collect_predictions() %>%
