...
Notice that there is no model yet: `Model: None`. It's time to specify the model we will use! Let's build a linear support vector machine (SVM) model. 

```{block2, type = "rmdnote"}
An SVM model can be used for either regression or classification, and linear SVMs often work well with text data. Even better, linear SVMs typically do not need to be tuned (see Section \@ref(tktk) for tuning model hyperparameters).
```
...
```{r firstregpredict, dependson="svmrs", fig.width=7, fig.height=5.5, fig.cap="Most Supreme Court opinions are near the dashed line, indicating good agreement between our SVM regression predictions and the real years"}
svm_rs %>%
  collect_predictions() %>%
...
```{r rfpredict, echo = FALSE, fig.width=7, fig.height=5.5, fig.cap="The random forest model did not perform very sensibly across years, compared to our first attempt using a linear SVM model"}
...
```{r snowballrmse, dependson="stopwordsres"}
...
This approach can be used to compare different lexicons and find the best one for a specific data set and model. Notice how the results for smart and stopword-iso are worse than removing no stopwords at all (remember that the RMSE was `r first_attemp_rmse` years in Section \@ref(firstregressionevaluation)). This indicates that removing a small stop word list is a good choice.
...
```{r ngramrs, dependson=c("fitngram")}
set.seed(123)
...
```{r lemmars, dependson=c("svmwf", "lemmarec", "scotusfolds")}
...
The best value for RMSE at `r collect_metrics(lemma_rs) %>% filter(.metric == "rmse") %>% pull(mean) %>% round(1)` shows us that using lemmatization can have a significant benefit for model performance, compared to `r first_attemp_rmse` from fitting a non-lemmatized linear SVM model in Section \@ref(firstregressionevaluation). However, this comes at a cost of much slower training because of the procedure involved in identifying lemmas; adding `step_lemma()` to our preprocessing increases the overall time to train the workflow by over tenfold.

```{block, type="rmdnote"}
We can use `engine = "spacyr"` to assign part-of-speech tags to the tokens during tokenization, and this information can be used in various useful ways in text modeling. One approach is to filter tokens to only retain a certain part-of-speech, like nouns. An example of how to do this is illustrated in this [**textrecipes** blogpost](https://www.hvitfeldt.me/blog/tidytuesday-pos-textrecipes-the-office/) and can be performed with `step_pos_filter()`.
```
...
```{block, type = "rmdwarning"}
There is _one_ tunable parameters in this model, the maximum number of tokens included in the model. 
```
...
```{r scotusfinaltunevis, dependson="finalscotusrs", fig.width=7, fig.height=7, fig.cap="Performance improves significantly at about 4000 tokens"}
final_rs %>%
  collect_metrics() %>%
...
```{r scotusfinalwf, dependson=c("chosenmade", "tunescotuswf")}
...
```{block, type = "rmdnote"}
Some of these features are unigrams and some are bigrams, but notice that none of either include stop words included in the Snowball lexicon because we removed them.
```
...
```{r scotusfinalpredvis, dependson="finalscotusrs", fig.width=6.5, fig.height=6, fig.cap="Predicted and true years from a linear SVM regression model with bigrams and unigrams, and stop words removed"}
final_fitted %>%
  collect_predictions() %>%
