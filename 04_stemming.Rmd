...
```{block2, type = "rmdnote"}
If you want to tokenize *and* stem your text data, you can try out the function `tokenize_word_stems()` from the **tokenizers** package, which implements Porter stemming just as we demonstrated here. For more on tokenization, see Chapter \@ref(tokenization).
...
```{r dependson="tidy_scotus", R.options=list(quanteda_print_dfm_max_ndoc = 0, quanteda_print_dfm_max_nfeat = 0), linewidth = 60}
tidy_scotus %>% 
...
```{r dependson="tidy_hca", R.options=list(quanteda_print_dfm_max_ndoc = 0, quanteda_print_dfm_max_nfeat = 0), linewidth = 60}
tidy_scotus %>%  
  mutate(stem = wordStem(word)) %>%
