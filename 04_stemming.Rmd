...
```{r porterlanguages, dependson="tidybylang", opts.label = "fig.long", fig.cap="Porter stemming results in five languages"}
tidy_by_lang %>%
...
```{r stemmingresults, dependson="stemming", fig.cap="Results for three different stemming strategies"}
stemming %>%
...
NLP practitioners who work with R can use this library via the [**spacyr**](http://spacyr.quanteda.io/) package [@Benoit19], the [**cleanNLP**](https://statsmaths.github.io/cleanNLP/) package [@Arnold17], or as an "engine" in the [**textrecipes**](https://textrecipes.tidymodels.org/) package [@textrecipes]. 
```
