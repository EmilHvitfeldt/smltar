...
The Danish alphabet appears, on the surface, similar to the one used in English with the addition of 3 more letters (æ, ø, and å) which come at the end of the alphabet. However, Danish doesn't use all the remaining letters natively. The letters C, q, w, x, and z are only used in words adopted from other languages, giving Danish a higher ratio of vowels to consonants since æ, ø, and å are all vowels. Danish has over 25 phonetically distinct vowel sounds [@konig2002germanic] which along with other language-specific features makes it hard to infer spelling from speech alone. One noticeable morphological difference coming from English to Danish is the use of compound nouns. Words such as "brandbil" (fire truck), "politibil" (police car) and "lastbil" (truck) all start with the morpheme "bil" (car) and prefixes with denoting the type of car. Because of this, some nouns seem more descriptive then their English counterpart; "vaskebjørn" (raccoon) splits into the morphemes "vaske" and "bjørn", literally meaning "washing bear". When working with Danish and other languages with compound words, such as German, it can provide benefits to apply compound splitting to extract more information [@Sugisaki2018].
