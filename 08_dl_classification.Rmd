...
We start by getting a pre-trained embedding. The glove embedding that we used in section \@ref(glove) will work for now. Setting `dimensions = 12` let up more easily compare the results with the previous sections.

```{r eval=FALSE}
library(textdata)

glove6b <- embedding_glove6b(dimensions = 12)
glove6b
```
...
```{r dense_model_evaluate, dependson="dense_pte_history", eval=FALSE}
dense_model_pte %>%
  evaluate(
    bake(prepped_recipe, kickstarter_test, composition = "matrix"),
    kickstarter_test$state
