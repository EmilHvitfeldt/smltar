...
```{block, type = "rmdwarning"}
When we include some dropout, we temporarily remove some units together with their connections from the network. The purpose of this is typically to reduce overfitting [@Srivastava2014]. Dropout is not exclusive to LSTM models, and can be used in many kinds of other network architectures. One way to add dropout to a network is by adding a dropout layer with `layer_dropout()`.
```
