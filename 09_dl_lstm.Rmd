...
After tokenizing, the preprocessing is different. We use `step_sequence_onehot()` to encode the sequences of words with integers representing each token in the vocabulary of `r scales::comma(max_words)` words, as described in detail in Section \@ref(onehotsequence). This is different than the representations we used in Chapters \@ref(mlclassification) and \@ref(mlregression), mainly because all the information about word sequence is encoded in this representation.
...
Figure \@ref(fig:rnndiag) depicts a high-level diagram of how the LSTM unit of a network works. In the diagram, part of the neural network, A, looks at some of the input and outputs a value. Doing this process some information is being held inside A to make the network "remember" this updated network A is then applied to the next input where it predicts new output and its memory is being updated.
...
```{block, type = "rmdwarning"}
When we include some dropout, we temporarily remove some units together with their connections from the network. The purpose of this is typically to reduce overfitting [@Srivastava2014].
```
...
An LSTM is actually a specific kind of recurrent neural network (RNN) [@ELMAN1990179]. Simple RNNs have feedback loops and hidden state that allow information to persist through steps but do not have memory cells like LSTMs. This difference between RNNs and LSTMs amounts to what happens in network A in figure \@ref(fig:rnndiag). RNNs tend to have a very simple structure, typically it is just a single tanh layer. This is much simpler than what happens in LSTMs.
...
```{r lstmheatmap, opts.label = "fig.square", fig.cap="Confusion matrix for final LSTM model predictions on testing set of Kickstarter campaign success"}
final_res %>%
