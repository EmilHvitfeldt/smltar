...
SVD is a common method for dimensionality reduction[@Golub1970]. SVD works intuitively by taking the data, which in our case is located in a high dimensional space, and rotating the axes such that the first dimension captures as much of the variance as possible. Keeping the first axis fixed, the remaining axes are rotated to maximize the variance in the second. This is repeated for all the dimensions.
More concrete will SVD factor the PMI matrix into a set of smaller matrices containing the word embeddings with dimension size we can control. The dimension size is typically picked to be in the low hundreds. Thus we get a matrix with dimension (`n_vocabulary * n_dim`) instead of (`n_vocabulary * n_vocabulary`), which can be a vast reduction in size for large vocabularies.
...
```{block, type = "rmdnote"}
`tidy_word_vectors` is not drastically smaller than `tidy_pmi` since the vocabulary only has a size of `r nrow(count(tidy_pmi, item1))` and `tidy_pmi` is represented in a sparse format.
```
...
SVD was used to represent the data in a way where each dimension contains the highest variance so far. We can now take a look at which and how much each dimension (tokens in this case) contributed to each of the resulting principal components that was produced using SVD.
...
```{block2, type = "rmdnote"}
The R package **wordsalad** [@R-wordsalad] provides a unified interface for finding different kinds of word vectors from text using pre-trained embeddings. The options include fastText, GloVe, and word2vec.
```
