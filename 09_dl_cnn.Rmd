...
metrics(val_res_double_dense, state, .pred_class)
```
...
metrics(val_res_double_conv, state, .pred_class)
```
...
We've come a long way in this chapter, and looked at the many different modifications to the simple CNN model we started with. Most of these modification didn't add much so this final model is not going to be much different then what we have seen so far.
...
We have been able to incrementally improve our model by adding to the structure and making good choices about preprocessing. We can visualize this final CNN model's performance using a confusion matrix as well, in Figure \@ref(fig:cnnmheatmap).

```{r cnnheatmap, fig.cap="Confusion matrix for final CNN model predictions on testing set of Kickstarter campaign success"}
final_res %>%
  conf_mat(state, .pred_class) %>%
  autoplot(type = "heatmap")
```
...
CNNs are a facinating type of neural networks that are able to detect patterns locally in the data. They essentially perform feature extraction which can then be used efficiently in latter layers of the network. Their simplicity and fast running time makes them excellent candiates for supervised models for text.

### In this chapter, you learned:

- how to preprocess text data for LSTM models
- about CNN network architectures
- how CNN layers can be stacked to extract patterns of varrying detail
- how byte-pair encoding can be used as a tokenizer fo finer detail
- how to do hyper parameter search in Keras with tfruns
