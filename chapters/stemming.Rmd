...
```{r stemmingresults, dependson="stemming", fig.width=10, fig.height=8}
stemming %>%
...
Our favorite way to implement lemmatization in R is the [cleanNLP](https://statsmaths.github.io/cleanNLP/) package [@Arnold17]. It provides R users access to, among other things, the excellent [spaCy](https://spacy.io/) library using a dataframe-centric design. Implementing lemmatization is slower and more complex than stemming. Just like with stemming, lemmatization often improves the true positive rate (or recall) but at the expense of the true negative rate (or precision) compared to not using lemmatization, but typically less so than stemming.
