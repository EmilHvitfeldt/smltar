...
An LSTM is a specific kind of network architecture with feedback loops that allow information to persist through steps^[Vanilla neural networks do not have this ability for information to persist at all; they start learning from scratch at every step.] and memory cells that can learn to "remember" and "forget" information through sequences. LSTMs are well-suited for text because of this ability to understand text as a sequence of words or characters, and can model structures within text like word dependencies. LSTMs are useful in text modeling because of this memory through long sequences; they are also used for time series, machine translation, and similar problems.
...
```{block, type = "rmdwarning"}
As we noted in Chapter \@ref(dldnn), the neural network model is modified **in place**; the object `lstm_mod` is different after we compile it, even those we didn't assign the object to anything. This is different from how most objects in R work, so pay special attention to the state of your model objects.
```
...
We used some keras defaults for model evaluation in the previous section, but we can take more control if we want or need to. Instead of using the `validation_split` argument, we can instead use the `validation_data` argument and send in our own validation set creating with rsample.
...
The overfitting has been reduced, and the difference between our model's performance on training and validation data is now smaller.
...
```{r}
val_res <- keras_predict(lstm_mod, kick_prep, assessment(kick_val$splits[[1]]), state)
val_res %>% metrics(state, .pred_class, .pred_prop)
```
...
lstm_auc <- val_res %>% 
  roc_auc(state, .pred_prop) %>% 
  pull(.estimate) %>% 
  round(3)
```

A regularized linear model trained on this dataset achieved results of accuracy of 0.684 and an AUC for the ROC curve of 0.752. This first LSTM with dropout is already performing better than such a linear model. We can plot the receive operator characteristic curve to evaluate the performance across the range of thresholds. 

```{r lstmvalroc}
val_res %>%
  roc_curve(state, .pred_prop) %>%
  autoplot()
```
...
An LSTM is actually a specific kind of recurrent neural network (RNN). Simple RNNs have feedback loops that allow information to persist through steps but do not have memory cells like LSTMs. RNNs are sometimes used in domains like speech recognition. 
...
Looks like more overfitting! We can see this by plotting the history as well.

```{r rnnhistory}
plot(rnn_history)
```

These results are pretty disappointing overall. Simple RNNs like the ones in this section can be challenging to train well, and just cranking up the number of embedding dimensions, units, or other network characteristics usually does not fix the problem. Often, RNNs just don't work well compared to simpler deep learning architectures like the dense network introduced in Section tktk [@Minaee2020], or even other machine learning approaches like regularized linear models with good preprocessing. 
...
```{r}
bilstm_res <- keras_predict(bilstm_mod, kick_prep, assessment(kick_val$splits[[1]]), state)
bilstm_res %>% metrics(state, .pred_class, .pred_prop)
```
...
```{r}
stacked_res <- keras_predict(stacked_mod, kick_prep, assessment(kick_val$splits[[1]]), state)
stacked_res %>% metrics(state, .pred_class, .pred_prop)
```
...
```{r}
padding_res <- keras_predict(padding_mod, padding_prep, assessment(kick_val$splits[[1]]), state)
padding_res %>% metrics(state, .pred_class, .pred_prop)
```

The same model architecture with default padding preprocessing resulted in an accuracy of `r lstm_acc` and an AUC of `r lstm_auc`; changing to `padding = "post"` has resulted in a remarkable degrading of predictive capacity.
...
```{block, type = "rmdwarning"}
Notice that we also shifted (subtracted) and scaled (divided) the `year` outcome by constant factors so all the values are close to one and centered around zero. Neural networks for regression problems typically behave better when dealing with outcomes that are roughly between -1 and 1.
```
...
## Case study: number of words and sequence length

In this chapter so far, we've used a longer sequence length of 50, compared to 30 in Chapter \@ref(dldnn), and we've worked with a vocabulary of 20,000 words or tokens. These are _hyperparameters_ of the model, and could be tuned as shown in Section tktk. Instead of tuning, let's try some smaller values, corresponding to faster preprocessing and model fitting but a less powerful model, and explore whether and how much it affects model performance. 
...
How did this smaller model, based on a smaller vocabulary and keeping shorter sequences in the model, perform?

```{r}
smaller_res <- keras_predict(smaller_mod, kick_prep, assessment(kick_val$splits[[1]]), state)
smaller_res %>% metrics(state, .pred_class, .pred_prop)
```

The original LSTM model with the larger vocabulary and longer sequences of 50 words had an accuracy of `r lstm_acc` and an AUC of `r lstm_auc`. Reducing the model's capacity to capture and learn text meaning by restricting its access to vocabulary and sequence information does result in a corresponding reduction in model performance. Notice that it is certainly 

```{block, type = "rmdnote"}
The relationship between these hyperparameters and model performance is weak, though. Notice that we cut the vocabulary in half and the sequence length by almost half, and saw almost modest reductions in accuracy.
```


## The full game: regression {#lstmfull}

We've come a long way in this chapter, even though we've focused on a very specific kind of RNN, the LSTM. Let's step back and build on final model, incorporating what we have been able to learn.

### Preprocess the data {#lstmfullpreprocess}

We know that we want to stick with the defaults for padding, and to use a larger vocabulary and sequence length for our final model. For this final model, we are not going to use our validation split again, so we only need to preprocess the training data.
...
Instead of using specific validation data that we can then compute performance metrics for, let's go back to specifying `validation_split = 0.25` and let the keras model choose the validation set.
...
```{r}
final_res <- keras_predict(final_mod, kick_prep, kickstarter_test, state)
final_res %>% metrics(state, .pred_class, .pred_prop)
```

This is our best performing model in this chapter on LSTM models, although not by much. We can again create an ROC curve, this time using the test data.
...
We have been able to incrementally improve our model by adding to the structure and making good choices about preprocessing. Notice that this final model still does not perform as well as the best models of Chapter \@ref(dldnn).

```{block, type = "rmdnote"}
For this dataset of Kickstarter campaign blurbs, an LSTM architecture is not turning out to give the best result compared to other options. However, LSTMs typically perform very well for text data and are an important piece of the text modeling toolkit.
```

For the Kickstarter data, these results are likely due to tktk...
...
- about resampling strategies for deep learning models (MOVE TO ANOTHER CHAPTER)
