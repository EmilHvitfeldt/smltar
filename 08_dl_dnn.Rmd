...
```{r dense_model_compiled, eval=!online}
dense_model %>% compile(
  optimizer = "adam",
...
```{r dense_model_history, eval=!online}
...
readr::write_rds(dense_history, "data/dnn_dense_history.rds")
```

```{r, include=FALSE, eval=online}
dense_history <- readr::read_rds("data/dnn_dense_history.rds")
```


We can visualize the results of the training loop by plotting the `dense_history` in Figure \@ref(fig:dense_model_history_plot).
...
```{r, include=FALSE, eval=!online}
keras::save_model_tf(dense_model, "data/dnn_dense_model")
readr::write_rds(val_history, "data/dnn_val_history.rds")
```

```{r, include=FALSE, eval=online}
dense_model <- keras::load_model_tf("data/dnn_dense_model/")
val_history <- readr::read_rds("data/dnn_val_history.rds")
```
...
Figure \@ref(fig:val_history_plot) still shows that significant overfitting at 10 epochs.

```{r val_history_plot, fig.cap="Training and validation metrics for dense network with validation set"}
plot(val_history)
```
...
```{r dnnroccurve, fig.width=7, fig.height=6, fig.cap="ROC curve for first DNN model predictions of Kickstarter campaign success"}
val_res %>%
...
```{r dense_model_pte, dependson="glove6b_matrix", eval=!online}
...
```{r dense_model_pte_weights, dependson="dense_model_pte", eval=!online}
dense_model_pte %>%
  get_layer(index = 1) %>%
...
```{r dense_pte_history, dependson="dense_model_pte_weights", eval=!online}
...
```{r, include=FALSE, eval=!online}
keras::save_model_tf(dense_model_pte, "data/dnn_dense_model_pte")
readr::write_rds(dense_pte_history, "data/dnn_dense_pte_history.rds")
```

```{r, include=FALSE, eval=online}
dense_model_pte <- keras::load_model_tf("data/dnn_dense_model_pte/")
dense_pte_history <- readr::read_rds("data/dnn_dense_pte_history.rds")
```
...
```{r dense_model_pte2, dependson="glove6b_matrix", eval=!online}
...
```{r dense_model_pte_noweights, dependson="dense_model_pte", eval=!online}
dense_model_pte2 %>%
  get_layer(index = 1) %>%
  set_weights(list(glove6b_matrix))
```
...
```{r dense_pte_noweights_history, dependson="dense_model_pte_noweights", eval=!online}
...
```{r, include=FALSE, eval=!online}
keras::save_model_tf(dense_model_pte2, "data/dnn_dense_model_pte2")
readr::write_rds(dense_pte2_history, "data/dnn_dense_pte2_history.rds")
```

```{r, include=FALSE, eval=online}
dense_model_pte2 <- keras::load_model_tf("data/dnn_dense_model_pte2/")
dense_pte2_history <- readr::read_rds("data/dnn_dense_pte2_history.rds")
```
...
```{r, include=FALSE, eval=!online}
readr::write_rds(cv_fitted, "data/dnn_cv_fitted")
```

```{r, include=FALSE, eval=online}
cv_fitted <- readr::read_rds("data/dnn_cv_fitted")
```
...
We can even create a visualization in Figure \@ref(fig:dnncvmetrics) to see the distribution of these metrics for these five folds.

```{r dnncvmetrics, fig.cap="Dense neutral network performance metrics for cross-validation folds"}
cv_fitted %>%
  unnest(validation) %>%
...
```{r alldnnroccurve, fig.width=7, fig.height=6, fig.cap="ROC curve for all DNN models' predictions of Kickstarter campaign success"}
all_dense_model_res %>%
  group_by(model) %>%
  roc_curve(truth = state, .pred_1) %>%
  autoplot() +
