...
The **tidymodels** framework of R packages is modular, so we can use it for certain tasks without committing to it entirely, when appropriate.
```
...
```{block, type = "rmdnote"}
This section serves to show how to use pre-trained word embeddings. We do not expect to see good results since the data and embeddings we will be trying do not match well. The main takeaway from this section should be that this approach is possible and how it is accomplished. Your task is to figure out if it is appropriate for your data and problem.
```
