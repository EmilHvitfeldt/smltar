...
```{block, type = "rmdpackage"}
**tokenizers** contains functions to split text into tokens of varying kinds. Including tokens such as words, letters, n-grams, lines and paragraphs.
```
...
```{block, type = "rmdpackage"}
**tidytext** provides functions to allow conversion of text to and from tidy formats. This allows us to work seamlessly with other tidy functions from **tidyverse** for further analysis.
```
...
```{r, eval=!knitr:::is_html_output(), echo=FALSE, out.width="100%"}
knitr::include_graphics("images/tokenization-emoji.png")
```
...
```{r ngramtokens, echo=FALSE, fig.cap="Using longer n-grams results in a higher number of unique tokens with fewer counts"}
