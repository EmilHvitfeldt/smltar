...
```{r tidybylang, dependson="tidyfirtree"}
...
```{r porterlanguages, dependson="tidybylang", fig.width=10, fig.height=7, fig.cap="Porter stemming results in five languages"}
tidy_by_lang %>%
...
We reduced the number of word features by many thousands, and the sparsity of our data by about 0.1%. Why is it possibly helpful to make our data more dense? Common sense says that reducing the number of word features in our dataset so dramatically will improve the performance of any machine learning model we train with it, *assuming that we haven't lost any important information by stemming*.
...
Check out the [steps of a Snowball stemming algorithm for German](https://snowballstem.org/algorithms/german/stemmer.html). 
```
...
```{r fir_tree_counts, dependson="tidyfirtree"}
...
```{r lemmafirtree, dependson="tidyfirtree", fig.width = 6, fig.height = 5, fig.cap="Results for lemmatization, rather than stemming"}
library(spacyr)
