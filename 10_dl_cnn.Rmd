...
The first neural networks we built in Chapter \@ref(dldnn) did not learn much about structure, sequences, or long-range dependencies in our text data. The LSTM networks we trained in Chapter \@ref(dllstm) were especially suited to learning long-range dependencies. In this final chapter, we will focus on **convolutional neural network** (CNN) architecture, which can learn local, spatial structure within a dataset.
...
```{r cnn-architecture, echo= FALSE, fig.cap="A template CNN architecture for one-dimensional input data. A sequence of consecutive CNN layers incremently reduces the tensor size, ending with single output value.", out.width="100%"}
...
```{r cnn-kernel-size, echo= FALSE, fig.cap="The kernel size affects the size of the resulting tensor. A kernel size of 3 uses the information from three values to calculate one value.", out.width="100%"}
...
keras::save_model_tf(simple_cnn_model, "data/cnn_simple_cnn_model")
readr::write_rds(cnn_history, "data/cnn_cnn_history.rds")
```

```{r, include=FALSE, eval=online}
simple_cnn_model <- keras::load_model_tf("data/cnn_simple_cnn_model/")
cnn_history <- readr::read_rds("data/cnn_cnn_history.rds")
```
...
```{r cnnroccurve, fig.width=7, fig.height=6, fig.cap="ROC curve for first CNN model predictions of Kickstarter campaign success"}
val_res %>%
...
```{r, include=FALSE, eval=!online}
keras::save_model_tf(cnn_double_dense, "data/cnn_cnn_double_dense")
```

```{r, include=FALSE, eval=online}
cnn_double_dense <- keras::load_model_tf("data/cnn_cnn_double_dense/")
```
...
```{r, include=FALSE, eval=!online}
keras::save_model_tf(cnn_double_conv, "data/cnn_cnn_double_conv")
```

```{r, include=FALSE, eval=online}
cnn_double_conv <- keras::load_model_tf("data/cnn_cnn_double_conv/")
```
...
```{r allcnnroccurve, fig.width=7, fig.height=6, fig.cap="ROC curve for three CNN variants' predictions of Kickstarter campaign success"}
all_cnn_model_predictions %>%
  group_by(model) %>%
  roc_curve(truth = state, .pred_1) %>%
  autoplot() +
...
```{r, include=FALSE, eval=!online}
keras::save_model_tf(cnn_bpe, "data/cnn_cnn_bpe")
readr::write_rds(bpe_history, "data/cnn_bpe_history.rds")
```

```{r, include=FALSE, eval=online}
cnn_bpe <- keras::load_model_tf("data/cnn_cnn_bpe/")
bpe_history <- readr::read_rds("data/cnn_bpe_history.rds")
```
...
```{block, type = "rmdwarning"}
Be sure that the preprocess function _matches_ the preprocessing that was used to train the model.
```
...
```{r limeplotfeatures, fig.cap="Plot of most important features for a CNN model predicting two observations."}
...
```{r limeplottextexplanations, fig.cap="Feature highlighting of words for two examples explained by a CNN model."}
...
```{r, include=FALSE, eval=!online}
readr::write_rds(runs_results, "data/cnn_runs_results.rds")
```

```{r, include=FALSE, eval=online}
runs_results <- readr::read_rds("data/cnn_runs_results.rds")
```
...
```{r, include=FALSE, eval=!online}
readr::write_rds(cv_fitted, "data/cnn_cv_fitted")
```

```{r, include=FALSE, eval=online}
cv_fitted <- readr::read_rds("data/cnn_cv_fitted")
```
...
```{r, include=FALSE, eval=!online}
keras::save_model_tf(final_mod, "data/cnn_final_mod")
readr::write_rds(final_history, "data/cnn_final_history.rds")
```

```{r, include=FALSE, eval=online}
final_mod <- keras::load_model_tf("data/cnn_final_mod/")
final_history <- readr::read_rds("data/cnn_final_history.rds")
```
...
```{r cnnfinalroc, fig.cap="ROC curve for final CNN model predictions on testing set of Kickstarter campaign success"}
final_res %>%
