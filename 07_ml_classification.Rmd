...
```{block, type = "rmdnote"}
This data set has many more than 50 of even the most common 5000 or more tokens, but it can still be good practice to specify `min_times` to be safe. Your choice for `min_times` should depend on your data and how robust you need your model to be.
```
...
```{r complaintsfinaltunevis, dependson="complaintsfinaltuners", fig.asp=1, fig.cap="Model performance is similar for the higher token options so we can choose a simpler model"}
autoplot(tune_rs) +
  labs(
...
```{r complaintsvip, dependson=c("complaintsfinalfitted", "complaintssplit"), fig.cap="Some words or bigrams increase a CFPB complaint's probability of being about credit reporting while some decrease that probability"}
library(vip)
...
```{block, type = "rmdnote"}
Most of these features are unigrams but a few are bigrams, like "subsection a". Notice that this bigram would have been removed if stop words were filtered out.
```
