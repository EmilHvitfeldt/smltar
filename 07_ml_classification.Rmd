...
```{r multiheatmap, opts.label = "fig.square", fig.cap="Confusion matrix for multiclass lasso regularized classifier, with most of the classifications along the diagonal"}
multi_lasso_rs %>%
  collect_predictions() %>%
...
```{r multiheatmapminusdiag, opts.label = "fig.square", fig.cap="Confusion matrix for multiclass lasso regularized classifier without diagonal"}
multi_lasso_rs %>%
  collect_predictions() %>%
  filter(penalty == best_acc$penalty) %>%
  filter(id == "Fold01") %>%
