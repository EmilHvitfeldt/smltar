...
The best value for ROC AUC from this tuning run is `r tune_rs_auc`. We can extract the best regularization parameter for this value of ROC AUC from our tuning results with `select_best()`.

```{r complaintsbestauc, dependson="complaintstuners"}
best_auc <- tune_rs %>%
  select_best("roc_auc")
...
```{r tunedlasso, dependson=c("complaintsbestauc", "complaintstuners")}
final_lasso <- finalize_workflow(tune_wf, best_auc)

final_lasso
```

Instead of `penalty = tune()` like before, now our workflow has finalized values for all arguments. The preprocessing recipe has been evaluated on the training data, and we tuned the regularization penalty so that we have a penalty value of `r round(best_auc$penalty, 5)`. This workflow is ready to go! It can now be fit to our training data.
...
We see here, for the best penalty we found, what terms contribute the most to a complaint _not_ being about credit. The words are largely about mortgages and other financial products.
