...
stop words can have different kinds of classifications in a corpus. We like to classify stop words into 3 groups: global, subject and document. Global stop words are words who almost always who are non-informative in the language as a whole, these are words such as "of" and "and" which are needed to glue the text together. These stopwords are very likely to be a safe bet to remove but they are few and far between. You are very likely to find some global stopwords in premade stop word lists \@ref(premadestopwords).
Next up is subject-specific stop words. These words are uninformative in the subject they are written in. Subjects can be broad like; financial, fiction, non-fiction and medical, but can also be more specific like; obituaries, health-code violations and job listings for librarians in Kansas.
Words like "bath", "bedroom" and "entryway" will generally not be considered stopwords, but they don't provide very much information in suburban house listings. You will most likely have to manually construct these stop word lists \@ref(homemadestopwords). These kinds of stop words where you can improve your performance if you have the domain expertise to create a good list.
Lastly, we have document stopwords. As the name suggests do these words not provide any information inside its own document. These are very hard to classify and won't be worth the trouble to find. Even if you can find document stop words then it is also not entirely obvious how that kind of information should be incorporated in a regression of classification task.
