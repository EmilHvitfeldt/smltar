...
We are using the same predictors as the first time and the way we are handeling `date_received` and `tags` will stay the same.
...
After exploring the data more in \ref(custumfeatures) it seems appropiate to add these features in our final model.
...
The tokenization and stipword removal will commence like normal, however this time will we count bi-grams instead of unigrams.
Last time we only included 100 tokens which isn't that many. 
...
We want the model to be robust and a token needs to appear a curtuin number of times before it turns from noise to signal.

```{block, type = "rmdnote"}
This dataset has much more then 50 of the most common 1000 tokens. It is still a good practice to specify to be safe. 50 is also an abetrary choice and needs to be picked depending on your preferences on the robustness of your model.
```
