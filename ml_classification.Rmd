...
In the last section, we talked about the many ways preprocessing affects the numerical representation of the text. 
In and the following chapters will we use these numerical representation of the data to aid the prediction of some quantity.
This chapter focuses on classification, which is the act of assigning an observation to one or more labels.
...
We will for the duration of this chapter limit ourselves to binary classification and will see if a complaint is about "Credit reporting, credit repair services, or other personal consumer reports" or not.
...
Next we remove stopwords with `step_stopwords()`, the default choice is the snowball stopword list, but custom lists can be provided too. Before we calculate the tf-idf we use `step_tokenfilter()` to only keep the 100 most frequent tokens, this is to avoid creating too many variables. To end off we use `step_tfidf()` to perform tf-idf calculations.
...
Here we have only kept the 100 most frequent tokens, we could have kept more tokens and a Naive Bayes model would be able to handle it okay, but we will limit it for this first time.
...
The default performance parameters for binary classification are accuracy and ROC AUC (area under the receiver operator curve). The accuracy is the percentage of accurate predictions. Which means that for this resample the average accurracy was `r collect_metrics(nb_rs) %>% filter(.metric == "accuracy") %>% pull(mean) %>% scales::percent_format(accuracy = 0.1)()`.
...
```{r firstroccurve, dependson="nbrs", fig.width=7, fig.height=6, fig.cap="ROC curve for resamples of US Consumer Finance Complaints."}
nb_rs_predictions %>%
  group_by(id) %>%
...
We see that the diagonal squares has darker shades then the off diagonal squares. This is a good sign since we are right more often then not. However the model still appears to be strugling since it is more or less at 50-50 when predicting something from the "Other" class.
...
```{r svmrs, dependson=c("svmwf", "complaintsfolds")}
...
We can now run `collect_metrics()` to extract and format the 3 performance metrics for the SVM model.
...
Let us finish by generating a confusion matrix.

```{r svmheatmap, dependson="nbrs"}
svm_rs %>%
  collect_predictions() %>%
  filter(id == "Fold01") %>%
  conf_mat(product, .pred_class) %>%
...
nb_rs <- fit_resamples(
  nb_wf,
  complaints_folds,
  metrics = metric_set(recal, precision)
)
```
