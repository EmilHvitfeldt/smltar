...
It is always a good idea to look at your data, as much as possible. Here are the first six complaints:
...
We are building a model from a dataset that includes annotations and labels added by the CFPB, but we need to ensure that only information that would be available at the time of prediction is included in the model.
...
The CFPB complaints dataset we have been working with has nine different `product` classes. In decreasing frequency, they are:
...
Figure \@ref(fig:censoredtrigram) shows the most frequent trigrams (Section \@ref(tokenizingngrams)) in our training dataset.
