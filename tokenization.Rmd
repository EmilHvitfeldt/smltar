...
One might want to have these ligatures separated back into separate characters. Before you do this you need to consider a couple of things. Firstly, you need to consider if the presence of ligatures is a meaningful feature to the question you are trying to answer. Secondly, there are two main types of ligatures, Stylistic and functional ligatures. Stylistic ligatures are when two characters are combined because the spacing between the characters has been deemed unpleasant. Functional ligatures like the German Eszett (also called the scharfes S, meaning sharp s) ß, is an official letter of the German alphabet. It is described as a long S and Z and historically has never gotten an uppercase glyph. This has led the typesetters to SZ or SS as a replacement when writing a word in uppercase. Additionally, ß is omitted entirely in German writing in Switzerland and is replaced with ss. Other examples are the "W" in the Latin alphabet (two "v" or two "u" joined together), æ, ø, and å in the nordic languages, are new letters. For historical reasons some place-names still use old spelling "aa" instead of å.

A newcomer in terms of characters is emoji. While they do look whimsical, various tricks have been used to more effectively store them in Unicode. Lets first look at the flag emoji. As of the time of writing, 261 different flags have emoji representation. However there they do not have individual Unicode characters. Let's take a look:
...
When tokenizing here we get to see the bare Unicode characters. Notice how each flag has two characters. If you were to look up the characters for the Japanese flag you will find that they are "REGIONAL INDICATOR SYMBOLS", "J" and "P" respectively. This is done such that all the flags can be represented only using 26 Unicode symbols. The same thing is done with the job emojis and gender modifiers, and general emojis with hairstyle modifiers, hair color modifiers and skin tone modifiers. There is a lot of information packed into emojis, and it is useful to remember to check that your tokenizer is treating them the way you would expect.
...
This sentence has several interesting parts which we need to decide whether to keep or to ignore when tokenizing. The first issue is the contraction in `"Don't"` which presents us with several possible options. The fastest option is to keep this as one word, but it could also be split up into `"do"` and `"n't"`. 
...
Another thing to consider is the degree of compression each type of tokenization provides. depending on the choice of tokenization you are going to get a different pool of possible tokens. The choice of tokenization can make an influence on your performance. By choosing a method that gives fewer possible tokens you allow later tasks to be performed faster, purely computationally. However, that comes with the risk of collapsing together buckets of different meaning tokens into one bucket. It is also worth noting that the spread of the number of different tokens varies with your choice of the tokenizer. Below is a visualization where these past points are illustrated. Each of the fairy tales from **hcandersenr** has been tokenized in 5 different ways and the number of distinct tokens has been mapped along the x-axis, notice that we are plotting on a log scale. We see that the number of distinct tokens goes down one once we lowercase the words or extract word stems. Secondly, we see that the distribution for Character tokenizers are really narrow since it is quite likely to use all the letters in the English alphabet.
