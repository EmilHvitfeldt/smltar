...
So far we have had problems with "fir-tree" being split up, we are going to fix this problem in two different ways. First we will be splitting on white spaces this is a decent way of finding workds, similary this doesn't split up hyponated words as the hyphon isn't considered a white-space. Next we will find a regex to match words with hyptons and extract those.
...
```{r}
str_split("This isn't a sentence with hyphonated-words.", "[:space:]")
```

And we nothing that is is doing pretty well. This version doesn't drop punctuation, but this can be achived by removing punctuation characters in the beginning and ends of words.
...
This regex is a little complicated so lets split it up a little. `^[:punct:]*` will look at the begining of the string (`^`) to look for punctuation character (`[:punct:]`) where it will select 1 or more (`+`). `[:punct:]+$` will look for punctuation character (`[:punct:]`) that appears 1 or more (`+`) and be in the end of the string (`$`). Lastly this will alternate (`|`) such that we get matches from both sides of the words. The reason we are using the quantifier `+` is because there are cases where a word is followed by multiple characters we dont want, such as `okay...` and `Really?!!!`. We are using `map()` since `str_split()` returns a list, and we want `str_remove_all()` to be applied to each element in the list, this example only have one element. 

```{block, type = "rmdnote"}
If you are in a situation where you want to avoid the dependensies that come with `purrr` you are able to use `lapply()` just as well. 
`lapply(str_remove_all, pattern = "^[:punct:]+|[:punct:]+$")`
```

Now we will see if we can get the same result by extraction. We will start of by constructing a regular expression that will capture the idea of a hyphonated word. Our definition is a word with 1 hyphon located inside. Since we want the hyphon to be inside the word will be need a non-zero number of character on either side of the hyphon. 

```{r}
str_extract_all("This isn't a sentence with hyphonated-words.", "[:alpha:]+-[:alpha:]+")
```

but wait, this only matched the hyponated word. This is happening because we are only matching words with hyphons, if we add the quantifier `?` then we can match 0 or 1 occurances.
...
Next we need to find out why `"a"` wasn't matched. If we look at the regular expression, we remember that we imposed the restriction that a non-zero number of characters needed to surround the hyphon place to avoid matching words start or ending with a hyphon, this means that the smallest possible pattern matched is 2 characters long. This can be fixed by using an alternation with `|`, we will keep our previous match on the lefthand side, and include `[:alpha:]{1}` on the righthand side to match the single length words that won't be picked up by the lefthand side. Notice how we arn't using `[[:alpha:]']` since we are not interested in matching single `'`'s.

```{r, eval=FALSE}
str_extract_all("This isn't a sentence with hyphonated-words.", "[[:alpha:]']+-?[[:alpha:]']+|[:alpha:]{1}")
```
...
```{r}
sentence <- c("This isn't a sentence with hyphonated-words.",
              "Same with this one")
...
We have shown how one can take some regular expressions to extract the tokens we want. however the code have so far been rather unstructored. We would ideally wrap these tasks into functions that can be used the same way `tokenize_words()` is used.

Lets start wirth the example with hyphonated words. To give it a little more spice lets add an option to turn all the words to lowercase.
...
We just need to make sure that the output is a list. Notice how we turned to lowercase before we did everything else as the remainding on the operations are case insensitive.
