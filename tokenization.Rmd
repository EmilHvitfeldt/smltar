...
Now we take the data and add chapter count by seeing when `CHAPTER` appears on the beginning of a line. Then we next the text colomn, apply our `add_paragraphs()` function and unnest again.
...
Now we have `r nrow(emma_paragraphed)` seperate paragraphs we can analyse. It is easy to see how we could go a step further to split by sentences, lines or words.
