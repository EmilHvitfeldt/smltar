...
In R, text is generally represented as a character vector, similar to strings in other languages. If we look at the first paragraph of *The Fir Tree*, we find the text of the story is in multiple character vectors, which themselves are a series of letters, spaces, and punctuation.
...
These vectors don't contain any information regarding what are words and what aren't. This is where the process of tokenization comes in.

In tokenization, we take an input (a string) and a type (a meaningful unit of text, such as a word) and split the input in to pieces (tokens) that correspond the type. [@Manning:2008:IIR:1394399]

![](images/tokenization/tokenization.jpg) TODO

Most commonly, the meaningful unit or type of token that we want to split text into units of is a **word**. However, it is  difficult to clearly define what a word is, for many or even most languages. Many languages do not use white space between words at all. Even languages that do use white space, including English, often have particular examples that are ambiguous [@Bender13]. Romance languages like Italian and French use pronouns and negation words that may better be considered prefixes with a space, and English contractions like "didn't" may more accurately be considered two words with no space.

As an exercise in understanding tokenization, let's start by defining a word as being any selection of alphanumeric (letter and numbers) symbols. Lets start by using some regex with `strsplit` to split the first 2 lines of The Fir Tree by anything non alphanumeric.
...
Tokenization will generally be one of first thing you do with the data, therefore it is very important to consider what happens in this first step of data reduction. As with most software we have a trade-off between speed and customizability as you will see in section \@ref(tokenization-benchmark). The fastest tokenization methods will give you little control over how it is done.

While the defaults works well it a lot of cases you will encounter situations where you might want to impose stricter roles to get better tokenized result. Consider the sentence "Don't forget you owe the bank $1 million for the house." which has a couple of interesting parts which we need to decide is worth keeping or fine to ignore. The first part is the contraction in "Don't" where we have a couple of different ways to go. The first and fastest way is to keep it as one word, but it could also be split up into  “do” and “n’t”. By performing the split we will be able to gain insight into both the amount of contractions as "n't" will be different then "not", but we will also have a broader reduction as the words "wouldn't" and "shouldn't" will be split according to the same pattern. Next issue at hand is how to deal with "\$1", the dollar sign is highly important part of this sentence as it denotes a mode of currency. Again we have the choice between removing and keeping, furthermore within the choice of keeping we can make the decision between keeping it as one or two tokens "\$1" or "\$" and "1". If we look at the default for `tokenize_words()` we notice that it defaults to removing punctuation which \$ is considered.
...
Lastly there is the question to keep the period as a sentence ender or not. By removing it we will not be able to locate the last word in a sentence using ngrams.
