...
We can plot these predictions to evaluate the performance across the range of dates. To make an interpretable plot, we need to convert our numeric representation for date back to R's date type, as we described in Section \@ref(firstdlregression).
...
This first LSTM model is still not working very well, and tends to predict high at early dates and low at later dates.
...
We may have expected this model to perform better than the regular LSTM but tktk...
...
```{block, type = "rmdwarning"}
Remember that a model like this is **modified in place**, so this process will fit the model for two epochs each on the five cross-validation folds, for a total of ten epochs. The function returns the transformed validation data for each split, but the model is being updated.
```
...
This padding strategy results in...
...
- Let's create a new validation split with a new seed from the training data, since we've been using the first one so much.
- Since padding _after_ the sequences appeared to work better, let's choose that padding strategy.
...
Let's increase the number of cross-validation folds to 10. If we don't change anything about the function `fit_split()`, this will mean we fit our model for 20 epochs.

```{r dojfolds}
set.seed(234)

doj_folds <- vfold_cv(doj_train, v = 10, strata = date)
doj_folds
```
...
- Our bidirectional LSTM did not perform well with a single validation split, but let's try it here with cross-validation folds.
