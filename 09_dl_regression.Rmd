...
The loss on the training data (called `loss` here) is significant better than the loss on the validation data (`val_loss`), indicating that we are overfitting.

```{r}
plot(rnn_history)
```

This may be hard to see from how this plot is printed, but the difference between the training and validation loss is lowest at around epoch 6 and then starts to increase, indicating _increasing_ overfitting. This is typical for neural networks without some protections in place; the longer you train, the more overfit your model will be. If we wanted to use this model, we should train it for fewer than ten epochs.
...
Let's also think about our model architecture. We saw evidence for overfitting, and we can counteract that by including dropout in our RNN, both in the regular sense (`dropout`) and in the feedback loops (`recurrent_dropout`). 
...
The loss, both for the training and validation data, is lower, indicating that the LSTM architecture is better for modeling these DOJ press releases than the RNN architecture. This is generally true for basically all text data.
...
We can compute any metrics for our validation data that we need to, because we have them available in `doj_assess` and `date_assess`.
...
These results are much better, but tktk...
...
This is our best performing model yet, and we are avoiding extreme overfitting by adding some dropout to the LSTM layers. tktk...
...
The bidirectional LSTM, able to learn both forward and backward text structures, is now tktk...
...
Adding another separate layer in the forward direction appears to have improved the network, more than extending the LSTM layer to handle information in the backward direction as with the bi-LSTM.
...
This model is still not performing even as well as a well-trained regularized linear model, and it takes much longer to train.
...
doj_folds <- vfold_cv(doj_train, v = 3, strata = date)
doj_folds
```
...
Let's use the same bi-LSTM from Section \@ref(mlregbilstm) but give it a different name for reading clarity. 
...
```{block, type = "rmdwarning"}
Remember that a model like this is **modified in place**, so this process will fit the model for two epochs each on the three cross-validation folds, for a total of six epochs. The function returns the transformed validation data for each split, but the model is being updated.
```
...
We can `map()` this function across all our cross-validation folds. The model will be trained for six epochs (2 each for the 3 folds).
...
Using cross-validation results in much better model performance compared to using a validation set alone. The amount of data we have in these DOJ press releases requires a more complex resampling strategy for fitting neural networks, rather than a single validation set.
...
Finally we have trained a deep learning model that performs better than a straightforward linear model. Remember that we reported in Section \@ref(dlregevaluation) that such a model has an RMSE of 0.0516 and an $R^2$ of 0.73. 

```{r cvpreds, fig.cap="Predicted and true dates for Department of Justice press releases using a bi-LSTM and cross-validation"}
plot_preds(cv_res)
```
...
This matrix has the same dimensions as `doj_matrix` but instead of padding with zeroes and truncating at the beginning of these DOJ press releases, this matrix is padded with zeroes at the end and truncates at the end.
...
This padding strategy results in noticeably worse performance.

```{r padpreds, echo = FALSE, fig.cap="Predicted and true dates for Department of Justice press releases with a different padding strategy"}
plot_preds(padding_res)
```
...
```{r cvbilstmmod}
bistack_mod <- keras_model_sequential() %>%
  layer_embedding(input_dim = max_words + 1, output_dim = 64) %>%
  bidirectional(layer_lstm(
    units = 32, dropout = 0.1, recurrent_dropout = 0.1, 
    return_sequences = TRUE
  )) %>%
  bidirectional(layer_lstm(
    units = 32, dropout = 0.1, recurrent_dropout = 0.1, 
...
```{r}
bistack_fitted <- doj_folds %>%
  mutate(validation = map(splits, fit_split, doj_prep, bistack_mod))
...
```{r}
bistack_metrics <- bistack_fitted %>%
  mutate(truth = map(splits, ~ assessment(.) %>% pull(date)),
         .pred = map(validation, ~ predict(bistack_mod, .)[,1]),
         rmse = map2_dbl(truth, .pred, rmse_vec),
         rsq  = map2_dbl(truth, .pred, rsq_vec))
...
```{r}
bistack_res <- tibble(date = date_assess,
                 .pred = predict(bistack_mod, doj_assess)[,1])

bistack_res %>% metrics(date, .pred)
```


```{r}
plot_preds(bistack_res)
```
