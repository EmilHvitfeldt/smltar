...
- Then, we tokenize (Chapter \@ref(tokenization)) the text of the court opinions. ~In this chapter, we are tokenizing via the [spaCy](https://spacy.io/) library as made available through the spacyr R package [@Benoit19]~.
...
- Next, we can remove a few columns with names that contains special characters problematic for some modeling functions.
...
## not running because of bug in parsnip: https://github.com/tidymodels/parsnip/issues/296
...
Instead of `penalty = tune()` like before, now our workflow has finalized values for all arguments. The preprocessing recipe has been evaluated on the training data, and we tuned the regularization penalty so that we have a penalty value of `r lowest_rmse$penalty`. This workflow is ready to go!
