...
```{r rfpredict, include = FALSE, fig.width=7, fig.height=6, fig.cap="The random forest model did not perform very sensibly across years, compared to our first attempt using a linear model with lasso regularization"}
...
```{r, dependson="snowballrmseoffline", include = FALSE, eval = !online}
list(snowball = snowball_rs, 
     smart = smart_rs, 
     `stopwords-iso` = stopwords_iso_rs) %>%
  map_dfr(show_best, "rmse", .id = "name") %>%
...
```{r snowballrmse, include = FALSE, fig.cap="Comparing model performance for predicting the year of Supreme Court opinions with three different stop word lexicons"}
word_counts <- tibble(name = c("snowball", "smart", "stopwords-iso")) %>% 
  mutate(words = map_int(name, ~length(stopwords::stopwords(source = .))))
...
```{r, dependson="ngramresoffline", include = FALSE, eval = !online}
bigram_rs %>%
  collect_metrics() %>%
  write_rds("data/ml_reg_bigram_metrics.rds")
```

```{r bigrammetrics, include = FALSE}
...
```{r, dependson="ngramrmseoffline", include = FALSE, eval = !online}
list(`1` = unigram_rs, 
     `1 and 2` = bigram_rs, 
     `1, 2, and 3` = trigram_rs) %>%
  map_dfr(collect_metrics, .id = "name") %>%
...
```{r ngramrmse, include = FALSE, fig.cap="Comparing model performance for predicting the year of Supreme Court opinions with three different degrees of n-grams"}
...
```{r lemmarecoffline, dependson="scotussplit", include = FALSE, eval = !online}
lemma_rec <- recipe(year ~ text, data = scotus_train) %>%
  step_tokenize(text, engine = "spacyr") %>%
  step_lemma(text) %>%
  step_tokenfilter(text, max_tokens = 500) %>%
...
```{r, dependson="scotuslemmavisoffline", include = FALSE, eval = !online}
lemma_rs %>%
  collect_metrics() %>%
  write_rds("data/ml_reg_lemma_metrics.rds")
```

```{r scotuslemmavis, include = FALSE, fig.width=8, fig.height=8, fig.cap="The best model using lemmatization is better than the best model without."}
...
```{r, dependson="lemmatunersoffline", include = FALSE, eval = !online}
lemma_rs %>%
  show_best("rmse") %>%
