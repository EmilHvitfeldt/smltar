...
tune_ngram <- function(ngram_options) {
  tune_grid(
    tunable_wf %>% add_recipe(ngram_rec(ngram_options)),
    scotus_folds,
    grid = grid_twenty
  )
}
...
Let's include six possible values for each parameter, for a combination of 36 models.
...
The tokens (unigrams or bigrams) that contribute in the positive direction, like "equal protection" and "sixth amendment", are associated with higher, later years and those that contribute in the negative direction, like "founded" and "dollars", are associated with lower, earlier years for these Supreme Court opinions. 

```{block, type = "rmdnote"}
Some of these features are unigrams and some are bigrams, but notice that none include stop words included in the Snowball lexicon because we removed them. The bigram "opinion court" likely mostly represents the phrase "opinion of the court".
```
