...
If you have already fit your model, you can still compute and explore non-default metrics as long as you saved the predictions for your resampled datasets using `control_resamples(save_pred = TRUE)`. Let's go back to the lasso model we tuned in Section \@ref(tunelasso), with results in `tune_rs`. We can compute the overall mean absolute percent error.
...
First, let's create the data preprocessing recipe. By setting the tokenization options to `list(n = 2, n_min = 1)`, we will include both unigrams and bigrams in our model. When we set `max_tokens = tune()`, we can train multiple models with different numbers of maximum tokens and then compare these models' performance to choose the best value.
...
Let's use the same lasso regression model specification we have used multiple times in this chapter, and set it up here again to remind ourselves. Using `mixture = 1` sets up a lasso model (`mixture = 0` would be a ridge model), and we will `tune()` the regularization penalty to find the best value.
...
Before we tune the model, we need to set up a set of possible parameter values to try. There are _two_ tuneable parameters in this model, the regularization parameter and the maximum number of tokens included in the model. Let's include seven possible values for each parameter, for a combination of 49 models.

```{r finalscotusgrid}
final_grid <- grid_regular(penalty(range = c(-4, 0)),
                           max_tokens(range = c(200, 1000)),
                           levels = 7)
...
Now it's time to set up our tuning grid. Let's save the predictions so we can explore them in more detail, and let's also set custom metrics instead of using the defaults. Let's compute RMSE, mean absolute error, and means absolute percent error during tuning.

```{r finalscotusrs, dependson=c("tunescotuswf", "scotusfolds", "finalscotusgrid")}
final_rs <- tune_grid(
  tune_wf,
  scotus_folds,
  grid = final_grid,
...
Now that all of the models with possible parameter values have been trained, we can compare their performance. Figure \@ref(fig:scotusfinaltunevis) shows us that

```{r scotusfinaltunevis, dependson="finalscotusrs", fig.width=8, fig.height=8, fig.cap="We see that..."}
final_rs %>%
  collect_metrics() %>%
  ggplot(aes(penalty, mean, color = .metric)) +
  geom_errorbar(aes(
...
We can also examine how the true and predicted years compare. Figure \@ref(fig:scotusfinalpredvis) shows us that

```{r scotusfinalpredvis, dependson="finalscotusrs", fig.width=8, fig.height=8, fig.cap="We see that..."}
final_rs %>%
  collect_predictions() %>%
  ggplot(aes(year, .pred, color = id)) +
  geom_abline(lty = 2, color = "gray80", size = 1.5) +
...
After we have that parameter, we can finalize our earlier tuneable workflow, by updating it with this value.
...
Notice that `final_wf` now has finalized values for `max_tokens` and `penalty`.
...
```{r scotusvip, dependson=c("scotusfinalwf", "scotussplit"), fig.width=8, fig.height=8, fig.cap="We see that..."}
library(vip)
...
Finally, let's return to our testing data. Notice that this is the first time we have used our testing data during this entire chapter; we tuned and compared models using resampled datasets instead of touching the testing set. We can use the function `last_fit()` to **fit** our model one last time on our training data and **evaluate** it on our testing data. We only have to pass this function our finalized model/workflow and our data split.
...
The metrics for the test set look about the same as the resampled training data and indicate we did not overfit during tuning.

TODO: Examine high performing/low performing observations?

## Summary

You can use regression modeling to predict a continuous variable from a dataset, such as a text dataset. Regularized linear models, such as lasso, often work well for text datasets, while tree-based models such as random forest often behave poorly. There are many possible preprocessing steps for text data, from removing stop words to n-gram tokenization strategies to lemmatization, that may improve your model. Resampling data sets and careful use of metrics allow you to make good choices among these possible options, given your own priorities.
